    ...
    public ResponseEntity<Void> spans(@RequestBody @NotBlank String data) {
        ...
        try {
            if(spanExporters == null){
                log.warn("A span was sent to the EUM-server for processing but no SpanExporter exists.");
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "No SpanExporter to process the request exists in the EUM server. To fix, update the EUM server's configuration at exporters.tracing to add a SpanExporter, e.g. for Jaeger.");
            }
